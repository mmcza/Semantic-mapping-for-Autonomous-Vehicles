...
class IoUMetric(Metric):
    def __init__(self, include_background=True, num_classes=4, original_size=(720, 1280)):
        ...
        self.add_state("iou_value", default=torch.tensor(0.0), dist_reduce_fx="mean")
    ...
    def update(self, preds, target):
        ...
        dice_value = self._dice_metric.aggregate().item()
        
        # Convert Dice coefficient to IoU
        # Formula: IoU = Dice / (2 - Dice)
        if dice_value == 1.0:
            self.iou_value = torch.tensor(1.0)  # Perfect overlap
        else:
            self.iou_value = torch.tensor(dice_value / (2 - dice_value))

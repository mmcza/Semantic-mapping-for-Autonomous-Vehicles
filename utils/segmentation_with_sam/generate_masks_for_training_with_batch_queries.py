...
def generate_annotations(input_dir, output_dir, model_name="sam2.1_hiera_small", save_visualizations=False, min_area=100, subset="default"):
    ...
    for group in category_groups:
        if "queries" in group:
            # Combine all queries for a group into a single string with space separation
    ...
    for image_path in tqdm(image_files, desc="Processing images"):
        try:
            ...
            for i in range(0, len(queries), QUERY_BATCH_SIZE):
                ...
                batch_images = [image_pil] * len(batch_queries)
                
                print(f"  Processing query batch {i//QUERY_BATCH_SIZE + 1}/{(len(queries) + QUERY_BATCH_SIZE - 1)//QUERY_BATCH_SIZE}: {batch_queries}")
                ...
                try:
                    ...
                    batch_results = model.predict(batch_images, batch_queries, box_threshold=0.35, text_threshold=0.3)
                    
                    # Process results for each query in the batch
                    for query_idx, query in enumerate(batch_queries):
                        result = batch_results[query_idx] if batch_results and len(batch_results) > query_idx else None
                        category_id = query_to_id.get(query, 1) #

                        # Check if we have results for this query
                        if result and isinstance(result, dict) and 'masks' in result and result['masks'] is not None:
                            masks = result['masks']
                            
                            # Handle case where masks might be a list without size attribute
                            if hasattr(masks, 'size') and masks.size > 0:
                                print(f"    - Query '{query}': Found {len(masks)} masks")
                                ...
                                print(f"    - Query '{query}': No valid masks found")
                        else:
                            print(f"    - Query '{query}': No masks returned")

                except Exception as batch_error:
                    print(f"    Error processing batch starting with query '{batch_queries[0]}': {str(batch_error)}")
                    continue
